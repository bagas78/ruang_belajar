	...
	<div class="card">
		...
		<div class="card-body">
			<form method="post" action="<?php echo base_url('soal/uraian_edit/'.$soal_uraian['soal_uraian_id']) ?>">
				...
				<?php foreach($uraian as $i => $var): ?>
					<div class="line">
						<hr>
					</div>
					<div class="form-group">
						<label class="form-control-label"><b>[ NO : <?= $i ?> ]</b> PERTANYAAN</label>
						<textarea placeholder="Masukkan pertanyaan" class="form-control" name="soal_uraian[<?= $i ?>][pertanyaan]"><?php echo $var['pertanyaan'] ?></textarea>

						<!-- gambar -->
						<input hidden type="text" class="gambar" name="soal_uraian[<?= $i ?>][gambar]" value="<?php echo @$var['gambar'] ?>">
